...
void BubbleCell(std::vector<InfoUtente> &utenti)
{
      int i, j, flag = 1;
      InfoUtente temp;
      int numLength = utenti.size( );
      for(i = 1; (i <= numLength) && flag; i++)
     {
          flag = 0;
          for (j=0; j < (numLength -1); j++)
         {
               if (utenti[j+1].cellapartenza < utenti[j].cellapartenza)
              {
                    temp = utenti[j];
                    utenti[j] = utenti[j+1];
                    utenti[j+1] = temp;
...
void BubbleSort(std::vector<InfoUtente> &utenti)
{
      int i, j, flag = 1;
      InfoUtente temp;
      int numLength = utenti.size( );
      for(i = 1; (i <= numLength) && flag; i++)
     {
          ...
          for (j=0; j < (numLength -1); j++)
         {
               if (utenti[j+1].peso > utenti[j].peso)
              {
                    temp = utenti[j];
                    utenti[j] = utenti[j+1];
                    utenti[j+1] = temp;
...
void BubbleSortMin(std::vector<InfoUtente> &utenti)
{
      int i, j, flag = 1;
      InfoUtente temp;
      int numLength = utenti.size( );
...
int pesolista(std::vector<InfoUtente> utenti)
{
	int peso = 0;
...
bool Aspetto(std::vector<InfoUtente> utentidivisi[2], LteEnbRrc* Nodo0, LteEnbRrc* Nodo1)
{
	for(int i=0;i<2;i++)
	{

		for(int j=0;j<utentidivisi[i].size();j++)
		{
			LteEnbRrc* Nodo = utentidivisi[i][j].cellapartenza == 0 ? Nodo0 : Nodo1;

			std::map<uint16_t, Ptr<UeManager> >::iterator it = Nodo->m_ueMap.begin();
			bool trovato = false;
			for (it= Nodo->m_ueMap.begin(); it!= Nodo->m_ueMap.end(); ++it)
			{
				if ( utentidivisi[i][j].nome==it->first)
				{
					trovato = true;
					break;
				}
...
std::vector<InfoUtente> utenti_record;
void do_handover( std::vector<InfoUtente> utenti, LteEnbRrc* enbRrc, int a , int b)

{


std::cout<<"puntatore "<<enbRrc;
std::cout<<"entra nel do handover";
#ifdef STAMPESTEF
	std::cout<<"i puntatori ai due nodi sono: \n";
#endif
	LteEnbRrc* triggatore[100];
	int nodi[100];
	std::map<int,LteEnbRrc* >::iterator it = enbRrc->enbtable_dentro.begin();
	int ct = 0;
	 for (it = enbRrc->enbtable_dentro.begin(); it!=enbRrc->enbtable_dentro.end(); ++it)
	     {
#ifdef STAMPESTEF
	      std::cout <<" "<< it->first << "  => " << it->second << '\n';
#endif

	 triggatore[ct]= it->second;
	 nodi[ct++]= it->first;
	}
	...
	int var=0;
	int conto=1;
	var=utenti_record[0].cellapartenza;
for(int k=1;k<utenti_record.size();k++){
if(var!=utenti_record[k].cellapartenza){

	//std::cout<<"stampo conto"<<conto<<"\n\n";
		conto++;
		var=utenti_record[k].cellapartenza;}


}
std::cout<<"stampo il conto delle presenze"<<conto<<"\n\n\n";

if (conto == enbRrc->enbtable_dentro.size()){
	int peso_totale=0;
	int media=0;
	for(int i=0;i< utenti_record.size();i++){
		peso_totale+= utenti_record[i].peso;}

			 media= peso_totale/conto;
			 std::cout<<"MEDIA"<<media<<"\n\n\n";
			 std::cout<<"peso totale "<<peso_totale<<"\n\n\n";


	 for(int k=0; k<utenti_record.size(); k++){
	 	//std::cout<<" valore cella partenza "<<utenti_record[k].cellapartenza<<"\n\n";
		 }


	std::vector<InfoUtente> utenti_divisi[400];

	for(int k=0; k<utenti_record.size(); k++)
	{
	      utenti_divisi[utenti_record[k].cellapartenza-1].push_back(utenti_record[k]);
	}
	...
	for(int i =0;i<conto;i++){
		for(int k =0;k<utenti_divisi[i].size();k++) {
		peso[i]+=utenti_divisi[i][k].peso;

		}//std::cout<<"peso[i]" <<peso[i]<<"\n\n";

		}

	for(int t=0; t<400;t++){
		for(int u=0; u<utenti_divisi[t].size(); u++){
			//std::cout<<"stampa di utenti divisi numero "<<t<<" utenti "<< utenti_divisi[t][u].nome <<" cellapartenza "<< utenti_divisi[t][u].cellapartenza<<"\n";

		}
	}


	std::vector<InfoUtente> appoggio;
		std::vector<int> minori;
		std::vector<int> maggiori;


		for(int h=0;h<conto;h++){
			if(peso[h]<media){minori.push_back(h);}
			 else
			 {


			   BubbleSortMin(utenti_divisi[h]);
			   maggiori.push_back(h);
			 }}

			int scarto = media /10;

			std::vector<int> destinazione;


			for(int i=0;i< maggiori.size();i++)
			{
			int appopre = 0;
			while(peso[maggiori[i]] > media - scarto)
			{
			    appoggio.push_back(utenti_divisi[maggiori[i]][appopre]);
			    peso[maggiori[i]]-= utenti_divisi[maggiori[i]][appopre].peso;
			    appopre++;
			}
			}

			BubbleSortMin(appoggio);

			int appo = 0;
			   for(int i=0;i< minori.size();i++)
			   {
			   while(peso[minori[i]] < media - scarto && appo < appoggio.size() -1)
			   {
			       //std::cout<<"MOUVI a " << minori[i];
			       destinazione.push_back(minori[i]);
			       peso[minori[i] ]+= appoggio[appo].peso;
			       appo++;
			   }
			   }

			   for(int i=destinazione.size() ;i< appoggio.size();i++)
			  			      {
			  			peso[appoggio[i].cellapartenza-1] += appoggio[i].peso;

			  			      }

			  			 for(int i =0;i<conto;i++){
			  			 			  			std::cout<<"peso di "<<i<<" è "<<peso[i]<<"\n\n\n";}

for(int i=0;i< destinazione.size();i++)
{

	std::cout<<"MOUVI da" << appoggio[i].cellapartenza<<  "a" <<destinazione[i]+1<<" l'utente "<<appoggio[i].nome<<"\n";
   //MOUVI da appoggio[i].cellapartenza a destinazione[i];

	LteEnbRrc* trig;
	std::map<int,LteEnbRrc* >::iterator jt ;
		 for (jt = enbRrc->enbtable_dentro.begin(); jt!=enbRrc->enbtable_dentro.end(); ++jt)
 	 	 {

			        //std::cout <<" "<< jt->first << "  => " << jt->second << '\n';
			       	if (appoggio[i].cellapartenza==jt->first){
			       							trig= jt->second;
			       	}
			       					 	}

			       					 //std::cout<<"triggatore è: "<<trig<<"\n\n";

			       					 std::map<uint16_t, Ptr<UeManager> >::iterator it = trig->m_ueMap.begin();
			       					 				for (it= trig->m_ueMap.begin(); it!= trig->m_ueMap.end(); ++it)
			       					 				{

			       					 					if ( appoggio[i].nome==it->first)
			       					 					{
			       					 						std::cout<<"sposto utente con RNTI: "<<appoggio[i].nome;
			       					 						std::cout<<" da : "<<appoggio[i].cellapartenza;
			       					 						std::cout<<" a : "<<destinazione[i]+1<<"\n";
			       					 						std::cout<<"Io che sposto sono il nodo con puntatore: "<<trig<<"\n";

			       					 						trig->DoTriggerHandover(appoggio[i].nome,destinazione[i]+1);}
			       					 					else std::cout<<"non ci sta l'utente nel nodo\n";}
	...
	/*
	for(int a=0;a<conto;a++){
	std::cout<<"minori "<< minori[a]<<"\n\n";}

	}}

	for(int a=0; a< minori.size();a++){
	while (peso[vettore_minori[a]]<media) // finche il peso del vettore dei minori è< media
	inserisci nel utenti_divisi[vettore_minori[a]] dal vettore di appoggio
	//ricalcolo peso
	for(k<utenti_divisi[vettore_minori[a]]){
	peso[vettore_minori[a]]+=utenti_divisi[vettore_minori[a]][k];
	}
...
/*for(int k=0; k<utenti_ordinati.size();k++){ scorro utenti divisi
 * for(int j=0; j< utenti.ordinati.size();j++){ scorro ogni utente diviso
 *
 * std::map<uint16_t, Ptr<UeManager> >::iterator it = Nodo->m_ueMap.begin();#
				for (it= Nodo->m_ueMap.begin(); it!= Nodo->m_ueMap.end(); ++it)
				{

					if ( utenti_ordinati[i][j].nome==it->first) se quell'utente è in quel nodo
					{}}
 *(vedi pure cellapartenza se no) cellapartenza=i+1 (utenti_ordinati[i])
 * allora niente
 * else se no invece vedi la sua cella partenza capisci in che nodo sta e spostalo a me
 * scorri enbtable;
 * controlla sempre se i nodi sono ancora in quell'enodeb#
 *
 *
 * }
 * }
...
for(int i =0;i<utenti.size();i++)
	{
	peso_totale+=utenti[i].peso;

	if(utenti[i].cellapartenza==0){
		peso_1+=utenti[i].peso;
	}else{

	peso_2+=utenti[i].peso;
	}

	}

if (conto == enbRrc->enbtable_dentro.size()){
	std::cout << "ENTRO IN CONTO \n\n";
std::cout<<"il peso totale e: "<<peso_totale<<"\n";
//std::cout<<"il peso 1 e: "<<peso_1<<"\n";
//std::cout<<"il peso 2 e: "<<peso_2<<"\n";
int perc;
perc= peso_totale/10;
std::cout<<"Valore di percentuale per ottenere l'equilibrio: "<<perc<<"\n";
if (peso_1>peso_2&& peso_1-peso_2<perc){
return;
std::cout<<"-----------Non sposto nulla perche ho raggiunto equilibrio---------------------------- \n";
}else
	if (peso_2>peso_1&& peso_2-peso_1<perc){
		std::cout<<"-----------Non sposto nulla perche ho raggiunto equilibrio----------------------------\n";
	return;}


		BubbleSort(utenti);
	std::vector<InfoUtente> utentidivisi[2];

	utentidivisi[0].push_back(utenti[0]);
	//std::cout << utenti[0].nome << " origine: " << utenti[0].cellapartenza <<"\n";

	for(int i =1;i<utenti.size();i++)
	{

		std::cout <<"RNTI: "<< utenti[i].nome << " appartenente al nodo: " << utenti[i].cellapartenza <<"\n";

		if(pesolista(utentidivisi[0]) < pesolista(utentidivisi[1]))
			utentidivisi[0].push_back(utenti[i]);
		else
			utentidivisi[1].push_back(utenti[i]);
	}
	...
	if(Aspetto(utentidivisi,triggatore[from[0]], triggatore[from[1]]))
	{
		return;
	}
	...
	for(int i=0; i<2; i++)
	{
		for(int j=0;j<utentidivisi[i].size();j++)
		{
			std::cout << "RTNI necessario :" << utentidivisi[i][j].nome << " su cella :" << i+1 << " con peso: " << utentidivisi[i][j].peso <<"\n";
		}
	}
	...
	for(int i=0; i<2; i++)
	{
		for(int j=0;j<utentidivisi[i].size();j++)
		{
			...
			if( utentidivisi[i][j].cellapartenza != i )
			{
				...
				int destination = -1;
				int cellapartenza = -1;
				if( utentidivisi[i][j].cellapartenza == 0)
				{
					Nodo = triggatore[from[0]];
					NodoTo = triggatore[from[1]];
					destination = nodi[from[1]];
					cellapartenza = from[0];
				}
				else
				{
					Nodo = triggatore[from[1]];
					destination = nodi[from[0]];
					NodoTo = triggatore[from[0]];
					cellapartenza = from[1];
				...
				for (it= Nodo->m_ueMap.begin(); it!= Nodo->m_ueMap.end(); ++it)
				{
					...
					if ( utentidivisi[i][j].nome==it->first)
					{
						std::cout<<"sposto utente con RNTI: "<<utentidivisi[i][j].nome;
						std::cout<<" da : "<<cellapartenza+1;
						std::cout<<" a : "<<destination<<"\n";
						std::cout<<"Io che sposto sono il nodo con puntatore: "<<Nodo<<"\n";

						Nodo->DoTriggerHandover(utentidivisi[i][j].nome,destination);

						//Nodo->RemoveUe(utentidivisi[i][j].nome);

						 for(int kk=Nodo->Vec_Name.size()-1;kk>=0;kk--)
						 {
							 if(Nodo->Vec_Name[kk] == utentidivisi[i][j].nome)
							 {
								 NodoTo->Vec_Name.push_back(Nodo->Vec_Name[kk]);
								 NodoTo->Vec_Size.push_back(Nodo->Vec_Size[kk]);
								 NodoTo->Vec_Ct.push_back(Nodo->Vec_Ct[kk]);

								 Nodo->Vec_Name.erase(Nodo->Vec_Name.begin()+kk);
								 Nodo->Vec_Size.erase(Nodo->Vec_Size.begin() + kk);
								 Nodo->Vec_Ct.erase(Nodo->Vec_Ct.begin() + kk);
...
void eraseut(std::vector<InfoUtente> &utenti_record, int id1 , int id2){
		 if(!utenti_record.empty()){
			 for(int h=utenti_record.size()-1; h>=0; h--)
			  { if(utenti_record[h].cellapartenza== id1){
				  utenti_record.erase(utenti_record.begin()+h);
			  }
		 ...
		 for(int h=utenti_record.size()-1; h>=0; h--)
					  { if( utenti_record[h].cellapartenza== id2){
						  utenti_record.erase(utenti_record.begin()+h);
					  }
	 ...
	 bool findme(std::vector<InfoUtente> vet, int rnti, int o ){
	 	bool trovato=false;
	 	for(int u=0; u<vet.size();u++ ){
	 	if(vet[u].cellapartenza==o){
	 		if(vet[u].nome== rnti){
trovato=true;
	 		 		return trovato;
	 		 		break;
	 	...
	 	trovato=false;return trovato;}





void eraseVec(std::vector<InfoUtente> utenti_record, int idcellaerase)
{
std::cout<<"idcellaerase" <<idcellaerase<<"\n";

 int start  = -1;
 int end = -1;
 for(int h= 0 ; h<utenti_record.size();h++)
 {
   if(utenti_record[h].cellapartenza==idcellaerase&& start < 0)
  {
    start = h;
   }
  else if(utenti_record[h].cellapartenza != idcellaerase && start >= 0 && end < 0 )
  {
    end = h;
    std::cout<<"end"<<end<<"\n";
   // break;
 ...
 if(start >= 0 && end > start)
  {
  utenti_record.erase(utenti_record.begin()+start,utenti_record.begin()+end);
 }
}



	 void stampaUtenti(std::vector<InfoUtente> vec)
	 {
	 for(int i =0;i< vec.size();i++)
	 {
	 std::cout << "cellapartenza: " << vec[i].cellapartenza << " nome: " << vec[i].nome << "\n";
	 }
	 }
...
NS_LOG_LOGIC ("enb1measurmentID = " << params.enb1MeasurementId);
NS_LOG_LOGIC ("enb2measurmentID = " << params.enb2MeasurementId);
NS_LOG_LOGIC ("targetCellId = " << params.targetCellId);
#ifdef STAMPESTEF
    std::cout<<"\n Sei all'interno della funzione ricevi X2"<<"\n";

    std::cout<<"Stai ricevendo nodi inviati da (source cellID): "<<params.enb2MeasurementId<<"\n";
    std::cout<<"Sei il nodo: "<<params.enb1MeasurementId<<"\n";
    //std::cout<<"Hai ricevuto questi valori dell' altro Nodo: \n";
#endif
//std::cout<<"stampo prima dell'erase \n";
BubbleCell(utenti_record);
//stampaUtenti(utenti_record);
eraseut(utenti_record,params.enb1MeasurementId,params.enb2MeasurementId );
std::vector<InfoUtente> Utenti;
//std::cout<<"stampo dopo l'erase \n";
//stampaUtenti(utenti_record);


for(int i=0;i< params.cellMeasurementResultList.size();i++)
{
	if (!findme(utenti_record,  params.cellMeasurementResultList[i].dlGbrPrbUsage, params.enb2MeasurementId )){
			//std::cout<<"inserisco valore mio proprio\n";
 InfoUtente U;
 U.peso = params.cellMeasurementResultList[i].ulGbrPrbUsage*params.cellMeasurementResultList[i].dlNonGbrPrbUsage;
 U.nome = params.cellMeasurementResultList[i].dlGbrPrbUsage;
 U.cellapartenza =params.enb2MeasurementId;

 utenti_record.push_back(U);
	}
		else break;
...
std::cout<<"fine utenti_record cancellati \n";

for(int i=0;i< Vec_Name.size();i++)
{

	if (!findme(utenti_record,  Vec_Name[i], params.enb1MeasurementId )){
		std::cout<<"inserisco valore mio proprio\n";
 InfoUtente U;
  U.peso = Vec_Ct[i]*Vec_Size[i] ;
  U.nome = Vec_Name[i] ;
  U.cellapartenza = params.enb1MeasurementId;
  utenti_record.push_back(U);
	}
	else break;

}
std::cout<<"lista finale di utenti record: \n";
 stampaUtenti(utenti_record);

//for(int t=0; t< utenti_record.size();t++){
	//	std::cout<<"utenti_record RNTI DOPO "<< utenti_record[t].nome <<" cellapartenza "<<utenti_record[t].cellapartenza<< "\n";
	//}

/*std::cout<<"quelli collegati pero sono: \n";
std::map<uint16_t, Ptr<UeManager> >::iterator it = m_ueMap.begin();
for (it= m_ueMap.begin(); it!= m_ueMap.end(); ++it){
std::cout<<" RNTI "<< it->first << " U => " << it->second << '\n';}*/
do_handover(Utenti, this,params.enb1MeasurementId,params.enb2MeasurementId);
