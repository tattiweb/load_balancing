...
class Packet;
    ...
    //Ptr<LteHelper> a = CreateObject<LteHelper> ();
 ...
 */
   ...
   // void do_handover( std::vector<InfoUtente> I,LteEnbRrc* enbRrc, int a);
    //bool Aspetto(std::vector<InfoUtente> utentidivisi, LteEnbRrc* enbRrc);
...
//public:std::map<uint64_t,uint32_t > rnti_utenti;

    public:int Rnti_altro;
    public:int Num_utenti_altro;
    public:int Cont_altro;
    public:int Size_altro;

    public:int Rnti_enb;
    public:int Size_enb;
    public:int Cont_enb;

    public:std::vector<uint16_t> Vec_Name ;
   public:std::vector<uint16_t> Vec_Ct ;
   public:std::vector<uint16_t> Vec_Size ;
   public:std::map<int, LteEnbRrc* > enbtable_dentro;

//public: void do_handover(int nr_utenti1, int Num_utenti_altro, int Rnti_altro, int Rnti_enb, int enb1MeasurementId, int enb2MeasurementId,int Size_altro, int Size_enb);
  ...
  virtual ~LteEnbRrc ();
    ...
    //EpcX2Sap::CellMeasurementResultItem cellMeasurementResultList;
...
public:
  ...
  enum LteEpsBearerToRlcMapping_t {RLC_SM_ALWAYS = 1,
                                   ...
                                   RLC_AM_ALWAYS = 3,
                                   PER_BASED = 4};
public: void DoTriggerHandover (uint16_t rnti, uint16_t targetCellId);
private:
...
public:std::map<uint16_t, Ptr<UeManager> > m_ueMap;
